...
int main()
{
    ...
    // cout << vowels[5] << endl; // 访问第6个元素 #此时编译器不会报错，运行也不报错
    
    // cout << vowels.at(5) << endl;      // 访问第6个元素 #此时编译器不会报错，但运行会报错 Aborted (core dumped)


    // vector <int> student_scores (3); // 3个整数的vector，初始化为0，使用的是默认构造函数
    // cout << student_scores[0] << endl; // 访问第1个元素

    vector <int> teacher_salary (3,100); // 3个整数的vector，初始化为100，使用的是带参数的构造函数
    cout << "老师的薪水为：" << teacher_salary[0] << endl; // 访问第1个元素
    ...
    cout << "添加新的成绩：";
    cin >> new_add_score;
    student_scores.push_back(new_add_score); //添加到vector的末尾
    cout << "再添加一个新的成绩：";
    cin >> new_add_score;
    student_scores.push_back(new_add_score);
    
    cout << "添加后,此时一共有" << student_scores.size() << "个成绩，分别为：" << endl;
